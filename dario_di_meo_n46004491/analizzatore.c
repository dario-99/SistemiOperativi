/*
    name: analizzatore
    descrizione: Pgm che si occupa di trasformare in uppercase una stringa ogni secondo, allocata in memoria condivisa di lunghezza N.
                 N e' passato nel vett di stringhe **argv.
*/
#include <stdio.h>
#include <unistd.h>
#include <stdlib.h>
...
int main(int argn, char **argv){
    ...
    if(chiave_buf < 0){
        perror("ERR ftok shm analizzatore\n");
        exit(1);
    }
    ds_shm = shmget(chiave_buf, (N + 1) + sizeof(int), IPC_CREAT | 0664);
    if(ds_shm < 0){
        perror("ERR shmget  analizzatore\n");
        exit(1);
    }
    buf = shmat(ds_shm, NULL, 0);
    if(buf == (void*)-1){
        perror("ERR shmat analizzatore\n");
        exit(1);
    }
    ...
    if(chiave_sem < 0){
        perror("ERR ftok sem analizzatore\n");
        exit(1);
    }
    ds_sem = semget(chiave_sem, 2, IPC_CREAT | 0664);
    if(ds_sem < 0){
        perror("ERR semget analizzatore\n");
        exit(1);
    }
